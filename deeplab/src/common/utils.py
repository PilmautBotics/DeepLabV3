...
from deeplab.models.segmentation.deeplabv3.layers import normalize, img_resize
...
def visualize_segmentation(image, mask, gt_mask=None, class_names=None, overlay=0.7, ignore_count_threshold=1, title=None, gt_title=None):
    ...
    # overlay images 
    dst = cv2.addWeighted(image, 0.5, mask_image, 0.7, 0)
    ...
    if gt_mask is not None:
        ...
        # plt.imsave('gt_mask_image.png', gt_mask_image)
        # plt.axis('off')
        # add plt title, optional
        #if gt_title:
        #    plt.title(gt_title)
