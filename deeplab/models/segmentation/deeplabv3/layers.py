...
def SepConv_BN(
    ...
    if depth_activation:
        x = ReLU()(x)
    x = DeeplabConv2D(filters, (1, 1), padding='same', use_bias=False, name=prefix + '_pointwise')(
        x
    )
...
def ASPP_block(x, OS):
    ...
    b4 = DeeplabConv2D(256, (1, 1), padding='same', use_bias=False, name='image_pooling')(b4)
    b4 = CustomBatchNormalization(name='image_pooling_BN', epsilon=1e-5)(b4)
    b4 = ReLU()(b4)
    b4 = Lambda(
        img_resize,
        arguments={'size': (feature_shape[1], feature_shape[2]), 'mode': 'bilinear'},
        name='aspp_resize',
    )(b4)
    ...
    x = DeeplabConv2D(256, (1, 1), padding='same', use_bias=False, name='concat_projection')(x)
...
def Decoder_block(x, skip_feature):
    ...
    x = Lambda(
        img_resize,
        arguments={'size': (skip_shape[1], skip_shape[2]), 'mode': 'bilinear'},
        name='decoder_resize',
    )(x)

    skip_feature = DeeplabConv2D(
        48, (1, 1), padding='same', use_bias=False, name='feature_projection0'
    )(skip_feature)
    skip_feature = CustomBatchNormalization(name='feature_projection0_BN', epsilon=1e-5)(
        skip_feature
    )
