...
from tensorflow.keras.layers import Add, Input, Lambda, ReLU, Reshape, Softmax
from tensorflow.keras.models import Model
from tensorflow.keras.utils import get_file, get_source_inputs

sys.path.append(os.path.join(os.path.dirname(os.path.realpath(__file__)), '..', '..'))
from deeplab.models.segmentation.deeplabv3.layers import (
    ASPP_block,
    ASPP_Lite_block,
...
def _inverted_res_block(
    ...
    x = ReLU(max_value=6.0, name=prefix + 'depthwise_relu')(x)
...
def MobileNetV2_body(input_tensor, OS, alpha, weights='imagenet'):
    ...
    x = CustomBatchNormalization(epsilon=1e-3, momentum=0.999, name='Conv_BN')(x)
    x = ReLU(6.0)(x)
    ...
    y = DeeplabConv2D(last_block_filters, kernel_size=1, use_bias=False, name='Conv_1')(x)
    y = CustomBatchNormalization(epsilon=1e-3, momentum=0.999, name='Conv_1_bn')(y)
    y = ReLU(6.0, name='out_relu')(y)
...
def Deeplabv3pMobileNetV2(
    input_shape=(512, 512, 3), alpha=1.0, weights='imagenet', input_tensor=None, num_classes=3, OS=8
):
    ...
    x = Lambda(
        img_resize,
        arguments={'size': (input_shape[0], input_shape[1]), 'mode': 'bilinear'},
        name='pred_resize',
    )(x)
...
if __name__ == '__main__':
    ...
    model, backbone_len = Deeplabv3pMobileNetV2(
        input_tensor=input_tensor, alpha=1.0, weights=None, num_classes=3, OS=8
    )
    model.summary()
