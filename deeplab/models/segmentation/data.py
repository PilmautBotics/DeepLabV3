...
from deeplab.src.common.data_utils import (
    normalize_image,
    random_blur,
    random_brightness,
    random_chroma,
...
class SegmentationGenerator(Sequence):
    def __init__(
        ...
        self.image_path_list = [
            os.path.join(dataset_realpath, 'images', image_id.strip() + '.png')
            for image_id in data_list
        ]
        self.label_path_list = [
            os.path.join(dataset_realpath, 'labels', image_id.strip() + '.png')
            for image_id in data_list
        ]
        ...
        self.batch_images = np.zeros(
            (batch_size, input_shape[0], input_shape[1], 3), dtype='float32'
        )
        self.batch_labels = np.zeros(
            (batch_size, input_shape[0] * input_shape[1], 1), dtype='float32'
        )
        self.batch_label_weights = np.zeros(
            (batch_size, input_shape[0] * input_shape[1]), dtype='float32'
        )
        ...
        for n, (image_path, label_path) in enumerate(
            ...
            if len(class_list):
                class_weights = class_weight.compute_class_weight(
                    class_weight='balanced', classes=class_list, y=label
                )
                class_weights = {
                    class_id: weight for class_id, weight in zip(class_list, class_weights)
                }
